...
void GENCODE::LogicalMultiplier_Branch1(const std::shared_ptr<NODE>& logical_multiplier)
{
	...
	if (*elem_1->m_value != *elem_2->m_value) {
		
		if (*elem_1->m_value == "variable-identifier") {
			throw std::string("GENCODE ERROR: elements for comparing must have the same type. But first element is " +
				*elem_1->m_value + ", second element is " + *elem_2->m_value + ". Line: "
				+ std::to_string(elem_1->m_children.front()->m_children.front()->m_line) + ", column: "
				+ std::to_string(elem_1->m_children.front()->m_children.front()->m_column));
		}
		else {
			throw std::string("GENCODE ERROR: elements for comparing must have the same type. But first element is " +
				*elem_1->m_value + ", second element is " + *elem_2->m_value + ". Line: "
				+ std::to_string(elem_1->m_children.front()->m_line) + ", column: "
				+ std::to_string(elem_1->m_children.front()->m_column));
		}
	}
